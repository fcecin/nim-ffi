...
proc ctxBindingGuard(
  ...
  #
  # Either shape can be a host thread's first entry: hosts routinely construct
  # on one thread and call from another. The body below unpacks the request on
  # the calling thread, which allocates via the GC, so the thread has to be
  # registered first. `initializeLibrary` does that and is idempotent. Raw AST,
  # not `quote`: `when declared` over an undeclared symbol inside `quote` ICEs.
...
proc exportedProc(
  ...
  # `cwireUnpack`/`cwirePack` alloc on the calling thread; `ctxBindingGuard` has
  # registered it with the GC by here. No teardown to match: it would free the
  # heap of a host thread that is still calling in.
